...
{
	FILE* trace[WORKLOAD_NUM];
	for (int i = 0; i < WORKLOAD_NUM; i++)
	{	trace[i] = fopen(argv[i + 1], "r");
		if (!trace[i])
			printf("Unable open the file!\n");
	}

	FILE* fp;
	fp = fopen(argv[WORKLOAD_NUM + 1], "w");
	if (!trace)
		printf("Unable open the file!\n");
	...
	while (1) {
		...
		do {
			do { workload = workload % WORKLOAD_NUM + 1; } while (finished[workload]);
			if (fscanf(trace[workload - 1], "%c %llx %u\n", &c, &tmp_num, &tmp_size) == EOF) {
				finished[workload] = 1;
			}
			else {
				tmp_num |= static_cast<unsigned long long>(workload) << 60; 
				break;
			}
			finished[0] = 1;
		} while (!finished[0]);
